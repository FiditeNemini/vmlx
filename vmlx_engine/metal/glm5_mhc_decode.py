"""Two-dispatch GLM-5.3 mHC transform for batch-one decode.
...
_PROJECTION_SOURCE = """
    uint row = thread_position_in_grid.x / PROJECTION_THREADS;
    ...
    for (uint col = tid; col < FEATURES; col += PROJECTION_THREADS) {
        value += float(streams[col]) * inv_rms *
...
_EPILOGUE_SOURCE = """
    ...
    if (tid == 0u) {
        ...
        for (uint index = 0u; index < H; ++index) {
            float pre_x = float(mix[index]) * scale_pre +
                float(hc_base[index]);
            pre_values[index] = 1.0f / (1.0f + metal::exp(-pre_x)) + epsilon;
            float post_x = float(mix[H + index]) * scale_post +
        ...
        for (uint row = 0u; row < H; ++row) {
            ...
            for (uint col = 0u; col < H; ++col) {
                ...
                float item = float(mix[2u * H + index]) * scale_comb +
    ...
    for (uint dim = tid; dim < D; dim += EPILOGUE_THREADS) {
        ...
        for (uint stream = 0u; stream < H; ++stream) {
            value += pre_values[stream] * float(streams[stream * D + dim]);
        }
        collapsed[dim] = T(value);
    }
    for (uint index = tid; index < H; index += EPILOGUE_THREADS) {
        post[index] = post_values[index];
    }
    for (uint index = tid; index < H * H; index += EPILOGUE_THREADS) {
        comb[index] = matrix[index];
    }
"""
...
def glm5_mhc_decode(
    ...
    *,
    ...
    if batch != 1 or rows != 1 or streams_count != 4 or hidden <= 0:
        return None
    ...
    mix = _projection_kernel()(
        ...
        grid=(_PROJECTION_THREADS * mix_size, 1, 1),
    ...
    return tuple(
        _epilogue_kernel()(
            ...
            grid=(_EPILOGUE_THREADS, 1, 1),
            threadgroup=(_EPILOGUE_THREADS, 1, 1),
            output_shapes=[
                (1, 1, streams_count),
                (1, 1, streams_count, streams_count),
                (1, 1, hidden),
            ],
