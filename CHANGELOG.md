...
- **Qwen3.8 Flash-Next: opt-in multi-row fusion for speculative verification** (`VMLX_QWEN4_GDN_GROUP_MAX_ROWS`, `VMLX_QWEN4_HC_COMPILE_MAX_ROWS`, default 1 = unchanged). The grouped GDN projection is bitwise identical at 2-4 rows; defaults move only after the interleaved A/B.
