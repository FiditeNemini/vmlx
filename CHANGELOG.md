...
- **Native MTP: the plain-decoding baseline is re-measured during the request, not only at its start.** The safety valve compared each window against a baseline seeded by one synchronized step at the start of the request and then scaled with the cycle wall, so a baseline that drifted over a long answer (context growth, a hotter chip) was only re-measured after a trip. The governor now runs a bounded calibration: when the windowed cycle wall has drifted 25% or more from its anchor, or 768 emitted tokens have passed since plain decoding was last measured, it decodes 8 tokens autoregressively at the current context, records that cost as the baseline, and returns to the depth it left (depth 1 and plain decoding remain reachable only through the valve). At most four calibrations run per request, so the cost is bounded at 32 plain-decoding tokens per answer; `VMLX_NATIVE_MTP_AR_CALIBRATION=0` disables it. The per-request AR-tier summary now reports the number of calibrations and the wall time spent in plain decoding, and `VMLX_NATIVE_MTP_CYCLE_TRACE=1` logs every cycle's wall, depth, acceptance and the baseline in force.
