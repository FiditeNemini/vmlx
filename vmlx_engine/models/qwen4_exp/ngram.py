...
class NGramHasher:
    ...
    def __init__(
        ...
        self.padded_vocab_size = (
            math.ceil(total / make_divisible_by) * make_divisible_by
        )
    ...
    def _shift_right_ignore_eos(self, token_ids: np.ndarray, shift: int) -> np.ndarray:
        ...
        gather_positions = np.broadcast_to(
            np.clip(source_positions, 0, None)[None, :], token_ids.shape
        )
    ...
    def hash_history(self, token_history: np.ndarray) -> np.ndarray:
        ...
        shifted = [
            self._shift_right_ignore_eos(token_history, s)
            ...
            for ngram in range(2, self.ngram_size + 1):
                ...
                for pos in range(1, ngram):
                    mixed = np.bitwise_xor(
                        mixed, shifted[pos] * self.layer_multipliers[pos]
                    )
                ids = np.remainder(
                    mixed[..., None], self._head_sizes_np[start:end][None, None, :]
                )
                blocks.append(ids + self._head_offsets_np[start:end][None, None, :])
        return np.concatenate(blocks, axis=-1)

    def hash_tokens(
        self, input_ids: np.ndarray, prev_context: np.ndarray | None = None
    ) -> np.ndarray:
