...
def test_unconfirmed_bundle_gets_verdict_but_no_stamp(tmp_path):
    ...
    assert plan["eligible"] is True
    assert plan["stamped"] is False
    # (c) config declares a DIFFERENT head layout than the runtime loaded
    _write_config(tmp_path, Q4_G128, model_type="qwen4_exp")
    plan = resolve_proposal_head_plan(tmp_path, Q8_G64, family="qwen4_exp")
    assert plan["stamped"] is False
    # (d) family/model_type cross-mismatch (27B config, qwen4_exp family)
...
def test_tied_embeddings_ineligible_and_confirm_checks_tie(tmp_path):
    ...
    assert plan["stamped"] is True
    # Config says untied but runtime loaded tied -> unconfirmed, no rewrite.
    (tmp_path / STAMP_FILENAME).unlink()
    _write_config(tmp_path, Q8_G64, model_type="qwen3_5", tied=False)
    plan = resolve_proposal_head_plan(tmp_path, tied, family="qwen3_5")
    assert plan["stamped"] is False
