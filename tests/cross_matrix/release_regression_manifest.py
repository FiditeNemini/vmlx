...
_ROWS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "proves": [
            ...
            "Diagnostic-only JANG DSV4 pool quant codec appends only newly generated CSA/HCA pool rows instead of requantizing the whole accumulated pool",
            "Diagnostic-only DSV4 pool quant reads reuse a materialized pool view instead of dequantizing and concatenating historical CSA/HCA pool segments on every read",
            "DSV4 product sessions default to full prefill and reject unsafe shortened-key composite stores",
            "DSV4 panel UI/env mapping hides product cache opt-in and keeps pool quant off by default",
