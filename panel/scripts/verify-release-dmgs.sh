...
# Verify the final public r18 DMG chain after signing, notarization, stapling,
...
EXPECTED_PRE_MANIFEST_SHA256="${VMLX_R18_PRE_NOTARY_MANIFEST_SHA256:-${VMLINUX_R18_PRE_NOTARY_MANIFEST_SHA256:-}}"
EXPECTED_FINAL_MANIFEST_SHA256="${VMLX_R18_FINAL_NOTARY_MANIFEST_SHA256:-${VMLINUX_R18_FINAL_NOTARY_MANIFEST_SHA256:-}}"
EXPECTED_SOURCE_COMMIT="${VMLX_R18_EXPECTED_SOURCE_COMMIT:-${VMLINUX_R18_EXPECTED_SOURCE_COMMIT:-}}"
EXPECTED_SOURCE_TREE="${VMLX_R18_EXPECTED_SOURCE_TREE:-${VMLINUX_R18_EXPECTED_SOURCE_TREE:-}}"
EXPECTED_PREFLIGHT_SHA256="${VMLX_R18_EXPECTED_PREFLIGHT_SHA256:-${VMLINUX_R18_EXPECTED_PREFLIGHT_SHA256:-}}"
...
require_pre_notary_handoff_environment() {
  ...
  for name in \
    ...
    if [[ -z "${!name}" ]]; then
      echo "ERROR: missing independent r18 build handoff value: $name" >&2
      exit 1
    fi
  done
}
...
require_final_notary_handoff_environment() {
  ...
  if [[ -z "$EXPECTED_FINAL_MANIFEST_SHA256" ]]; then
    echo "ERROR: VMLX_R18_FINAL_NOTARY_MANIFEST_SHA256 is required from the notarization handoff" >&2
    exit 1
  fi
}

require_r18_release_context() {
  assert_unshadowed_apple_tools
  if [[ "$PATH" != "$R18_FIXED_PATH" ]]; then
    echo "ERROR: r18 verification PATH is not sanitized" >&2
    exit 1
  fi
  if [[ -n "${VMLX_RELEASE_OUTPUT_DIR:-}" || -n "${VMLINUX_RELEASE_OUTPUT_DIR:-}" ]]; then
    echo "ERROR: r18 verification output is fixed at $PANEL_DIR/release; overrides are forbidden" >&2
    exit 1
  fi
  if [[ "$VERSION" != "1.6.18" ]]; then
    echo "ERROR: the r18 artifact-chain verifier requires package version 1.6.18, found $VERSION" >&2
    exit 1
  fi
  if [[ "$RELEASE_SCOPE" != "r18_production" ]]; then
    echo "ERROR: set VMLX_RELEASE_SCOPE=r18_production for final r18 verification" >&2
    exit 1
  fi
  ...
  if [[ -z "$PRIVATE_EVIDENCE_ROOT" ]]; then
    echo "ERROR: VMLX_R18_PRIVATE_EVIDENCE_ROOT is required and must be outside every Git worktree" >&2
    exit 1
  fi
...
verify_final_release_chain() {
  require_r18_release_context
  require_final_notary_handoff_environment
  local final_manifest="${VMLX_R18_FINAL_NOTARY_MANIFEST:-${VMLINUX_R18_FINAL_NOTARY_MANIFEST:-$PRIVATE_EVIDENCE_ROOT/vmlx-${VERSION}-r18-post-notary-manifest.json}}"
  if [[ -n "${VMLX_R18_FINAL_NOTARY_MANIFEST:-}" ]] \
    && [[ -n "${VMLINUX_R18_FINAL_NOTARY_MANIFEST:-}" ]] \
    && [[ "$VMLX_R18_FINAL_NOTARY_MANIFEST" != "$VMLINUX_R18_FINAL_NOTARY_MANIFEST" ]]; then
    echo "ERROR: VMLX_R18_FINAL_NOTARY_MANIFEST and VMLINUX_R18_FINAL_NOTARY_MANIFEST disagree" >&2
    exit 1
  fi
  ...
  artifact_chain check-final \
    ...
    --expected-source-tree "$EXPECTED_SOURCE_TREE" \
    --expected-preflight-sha256 "$EXPECTED_PREFLIGHT_SHA256"
  echo "==> Final r18 DMG artifact chain verified: $final_manifest"
}
