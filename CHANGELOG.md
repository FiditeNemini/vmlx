...
- **On-the-fly AR safety valve for native MTP, at every depth policy.** Fixed-depth MTP sessions (the new Qwen3.8 default) previously ran with no fallback at all: a request whose speculative decoding fell below plain autoregressive speed stayed slow for the whole response. Every verify cycle now compares the recent windowed cost per token against the request's own measured autoregressive step, scaled for context growth, and drops the request to plain decoding within about a window when speculation is genuinely losing. It ignores a single stalled cycle, never fires during warmup, is per-request (correct under batching and identical for chat and API), and can be disabled with `VMLX_NATIVE_MTP_AR_SAFETY=0`.
