...
- **Qwen3.8 Flash-Next: grouped GDN projections and compiled hyper-connections can cover speculative verification** (2-4 rows) via `VMLX_QWEN4_GDN_GROUP_MAX_ROWS=4` / `VMLX_QWEN4_HC_COMPILE_MAX_ROWS=4`. The grouped projection is bitwise identical to the separate ones and measured +0.3-0.9% at fixed depth 3 with byte-identical outputs across 1.7k-26k context; the default stays decode-only until the tool-continuation receipts are in.
...
- **Qwen3.8 Flash-Next: the fused expert pair kernel no longer runs on the speculative draft head.** With the single-row pair kernel registered on the draft head's expert layer, fixed depth-3 decoding on 4M ran 39 / 43 / 34 tok/s at 1.7k / 6.6k / 26k context against 52 / 46 / 37 with it off, at identical acceptance; plain decoding keeps the kernel's +3%. The head's modules now stay on the stock path (`VMLX_QWEN4_FUSED_MOE_PAIR_MTP_HEAD=1` restores).
- **Qwen3.8 Flash-Next: opt-in fused expert pair kernel for mixed-width bundles** (`VMLX_QWEN4_FUSED_MOE_PAIR_MIXED=1`). 4S carries 2-bit gate with 3-bit up projections and 6S 4-bit with 6-bit, layouts the kernel could not unpack, so it silently kept the stock path on those bundles. The generic path handles per-projection widths including 3- and 6-bit; measured +2.1% on 4S plain decoding with identical outputs. Default off pending the depth-3 receipts.
...
- **Native MTP: measured depth economics.** At long context a depth-3 verify (4 rows) grows faster than a depth-1 one: on 4S at 26k tokens depth 1 ran 18-19 ms/token where fixed depth 3 ran 26.5 (plain decoding 27.5). After the configured depth has one judged window the multimodal lane runs one depth-1 window against the configured depth's measured cost and keeps whichever wins by 10%, re-trying the other with backoff (two comparisons per request). Configured depth remains the starting depth and the ceiling.
