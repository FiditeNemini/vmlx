...
# Submit immutable private snapshots of the exact r18 build-manifest DMGs to
...
PRE_NOTARY_MANIFEST="${VMLX_R18_PRE_NOTARY_MANIFEST:-${VMLINUX_R18_PRE_NOTARY_MANIFEST:-}}"
FINAL_NOTARY_MANIFEST="${VMLX_R18_FINAL_NOTARY_MANIFEST:-${VMLINUX_R18_FINAL_NOTARY_MANIFEST:-}}"
...
if [[ -n "${VMLX_R18_PRE_NOTARY_MANIFEST:-}" ]] \
  && [[ -n "${VMLINUX_R18_PRE_NOTARY_MANIFEST:-}" ]] \
  && [[ "$VMLX_R18_PRE_NOTARY_MANIFEST" != "$VMLINUX_R18_PRE_NOTARY_MANIFEST" ]]; then
  echo "ERROR: VMLX_R18_PRE_NOTARY_MANIFEST and VMLINUX_R18_PRE_NOTARY_MANIFEST disagree" >&2
  exit 1
fi
...
notarize_release_chain() {
  ...
  if [[ -z "$PRE_NOTARY_MANIFEST" ]]; then
    echo "ERROR: VMLX_R18_PRE_NOTARY_MANIFEST is required from the build handoff" >&2
    exit 1
  fi
  ...
  export VMLX_R18_FINAL_NOTARY_MANIFEST="$FINAL_NOTARY_MANIFEST"
  export VMLX_R18_FINAL_NOTARY_MANIFEST_SHA256="$final_sha256"
  ...
  printf 'VMLX_R18_FINAL_NOTARY_MANIFEST=%s\n' "$FINAL_NOTARY_MANIFEST"
  printf 'VMLX_R18_FINAL_NOTARY_MANIFEST_SHA256=%s\n' "$final_sha256"
