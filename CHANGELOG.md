...
- **Native MTP: measured depth economics, and what "fixed" depth means.** At long context a depth-3 verify (4 rows) grows faster than a depth-1 one: on 4S at 26k tokens depth 1 ran 18-19 ms/token where fixed depth 3 ran 26.5 (plain decoding 27.5). After the configured depth has one judged window the multimodal lane runs one depth-1 window against the configured depth's measured cost and keeps whichever wins by 10%, re-trying the other with backoff (two comparisons per request). Under both depth policies the configured depth is the starting depth and the ceiling; `fixed` only means no acceptance-based adaptation. `VMLX_NATIVE_MTP_DEPTH_PROBE=0` disables the comparison and, together with `VMLX_NATIVE_MTP_AR_SAFETY=0`, pins the depth exactly. The per-request finish line now reports cycles spent at each depth (`cycles_by_depth`).
